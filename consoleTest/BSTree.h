...
{   
    BSTNode<T> *x = NULL; // 待删除节点的子节点
    BSTNode<T> *y = NULL; // 待从内存中删除的节点
    
    // 如果待删除节点左节点或者右节点为空，那么表示，可以直接Z，将Z的孩子连接到Z的父节点上
    if ((z->left == NULL) || (z->right == NULL))
        y = z;
    else
        // 如果左右节点都不为空，则表示，必须寻找后继节点，
        // 且删除方案调整为将Z指向的节点可以替换为后继节点的key，然后再删除后继节点
        // 因为后继节点的左海子绝对为空
        y = successor(z);
